...
def update_data_service():
    try:
        ...
        print("Error:", e)
        return{"status": 500, "message": f'Error updating data: {e}'}
...
def predict_data_service():
    try:
        ...
        app.local_df, app.metadata_df, prediction_count, copy_count = predict_column(app.local_df, prediction_column, app.metadata_df)
        print('Done Predicting')
        return {'status': 200, 'message':{prediction_column: {'Predictions': prediction_count, 'Copied': copy_count}}}
    except Exception as e:
        print('Error', e)
...
def open_pandas_service():
    try:
        ...
        diff = app.local_df.compare(gui_df)
        print('Done Checking for changes')
        print(diff.columns)
        print(diff.columns[0])
        print(diff)
        print('----------------')
        for i in diff.index:
            print(diff.loc[i])
            for x in range(len(diff.loc[i])):
                if x % 2 == 1:
                    app.local_df.loc[i, diff.columns[x][0]] = diff.loc[i][x]
                    app.metadata_df.loc[i, diff.columns[x][0]] = 'local'
